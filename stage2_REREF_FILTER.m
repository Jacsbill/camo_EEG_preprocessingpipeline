...
%%%%%%% added notch filter and also added remove channel code  %%% dont use
%%%%%%% - crap
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     [ALLEEG EEG CURRENTSET ALLCOM] = eeglab;
    ...
    [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 ); EEG = eeg_checkset( EEG );
     ...
     fprintf(fileID, '%s\n',datestr(t));
     fprintf(fileID,'%f %f\n',chm);
     EEG = pop_reref( EEG, [],'exclude',det.chnrm );
     saveit=strcat([det.subjects{n} '_removedchansatrerefstage.mat'])
     save(saveit,'chm')
     ...
     EEG = eeg_checkset( EEG );
     EEG = pop_saveset( EEG, 'filename',strcat([det.subjects{n} '_import_rr.set']),'filepath',subject_analyse);
%      [ALLEEG EEG CURRENTSET ALLCOM] = eeglab;
     EEG = pop_loadset('filename',strcat([det.subjects{n} '_import_rr.set']),'filepath',subject_analyse);
     [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
     EEG = pop_firws(EEG, 'fcutoff', det.HP, 'ftype', 'highpass', 'wtype', 'kaiser', 'warg', 5.65326, 'forder', 1856, 'minphase', 0);
     [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3,'setname',strcat([det.subjects{n} '_import_rr_filt.set']),'gui','off'); 
      ...
      %%% noth filter - much pooer data
%        EEG  = pop_basicfilter( EEG,  1:70 , 'Boundary', 'boundary', 'Cutoff',  50, 'Design', 'notch', 'Filter', 'PMnotch', 'Order',  180 ); % GUI: 24-May-2019 16:49:26
     [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4,'setname',strcat([det.subjects{n} '_import_rr_filt.set']),'gui','off'); 
     EEG = eeg_checkset( EEG );
     EEG = pop_saveset( EEG, 'filename',strcat([det.subjects{n} '_import_rr_filt.set']),'filepath',subject_analyse);
