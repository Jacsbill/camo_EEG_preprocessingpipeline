...
%%%%%%% This is for 1hz data. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

addpath (det.root)
for n=1:length(det.subjects);
    subject=det.subjects{n};
    subject_analyse1=fullfile(det.rootstudy,det.subjects{n});
    subject_analyse=fullfile(det.rootstudy,det.subjects{n},'response_epoched_files');
     subject_analyseEX=fullfile(det.rootstudyEX,det.subjects{n});
    addpath(det.rootstudy);
    fileID2 = fopen('FASTER_notesfilt1.txt','a');
    epochinfo=fullfile(det.rootstudy, det.e_list);
    cd(subject_analyse1)
   
%     [ALLEEG EEG CURRENTSET ALLCOM] = eeglab;
     EEG = pop_loadset('filename',strcat([det.subjects{n} '_import_rr_filt_ICArm.set']),'filepath',subject_analyseEX);
...
%     EEG  = pop_editeventlist( EEG , 'AlphanumericCleaning', 'on', 'BoundaryNumeric', { -99}, 'BoundaryString', { 'boundary' }, 'List', 'C:\levEEGanalysis\main_eventcodes2.txt', 'SendEL2', 'EEG', 'UpdateEEG', 'askUser', 'Warning', 'on' ); % GUI: 20-Aug-2021 10:41:47

    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 2,'setname',strcat([det.subjects{n} '_import_rr_filt_elist.set']),'gui','off'); 
    EEG = eeg_checkset( EEG );
    EEG = pop_saveset( EEG, 'filename',strcat([det.subjects{n} '_import_rr_filt_ICArm_elist.set']),'filepath',subject_analyse);
...
%         [ALLEEG EEG CURRENTSET ALLCOM] = eeglab;
        EEG = pop_loadset('filename',strcat([det.subjects{n} '_import_rr_filt_ICArm_elist.set']),'filepath',subject_analyse);
%         [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
        EEG = pop_epoch( EEG, [ det.epochs], det.epochlength, 'newname',  strcat([det.subjects{n} '_import_rr_filt_ICArm_elist.set']), 'epochinfo', 'yes');
        ...
        EEG = eeg_checkset( EEG );
        [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'setname',strcat([det.subjects{n} '_import_rr_filt_elist_epoched.set']),'gui','off'); 
        EEG = pop_saveset( EEG, 'filename', strcat([det.subjects{n} '_import_rr_filt_elist_epoched.set']),'filepath',subject_analyse);
        [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
        eeglab redraw;
        ...
        diary (epochdiary)
        EEG = pop_loadset('filename',strcat([det.subjects{n} '_import_rr_filt_elist_epoched.set']),'filepath',subject_analyse);
        [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
        EEG = eeg_checkset( EEG );
        cfg=[];
        cfg.datachan=1:64;
        ...
        [EEG,trials2remove,comps2remove,gen_bad_chans]=eegF_FASTER(cfg,EEG)
          ...
          save Faster_BCfilt1 gen_bad_chans
          save Faster_BTfilt1 trials2remove
          save Faster_threshfilt1 trh
          cd ..
          t = datetime('now')
        ...
        [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'setname',strcat([det.subjects{n} '_import_rr_filt_ICArm_elist_FASTER.set']),'gui','off'); 
        EEG = pop_saveset( EEG, 'filename', strcat([det.subjects{n} '_import_rr_filt_ICArm_elist_FASTER.set']),'filepath',subject_analyse);
        
        %%%%% save the file without removing trials
        EEG = pop_loadset('filename',strcat([det.subjects{n} '_import_rr_filt_ICArm_elist_FASTER.set']),'filepath',subject_analyse);
        [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
        EEG = eeg_checkset( EEG );
        ...
        [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'setname',strcat([det.subjects{n} '__import_rr_filt_ICArm_elist_FASTER_REMOVED.set']),'gui','off'); 
        EEG = eeg_checkset( EEG );
    
        EEG = pop_saveset( EEG, 'filename',strcat([det.subjects{n} '__import_rr_filt_ICArm_elist_FASTER_REMOVED.set']),'filepath',subject_analyse);
        [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
         EEG = eeg_checkset( EEG );
        eeglab redraw;
        diary off
